        ...
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                HStack(spacing: 5) {
                    ...
                    // Ashura: catalog status badge (maintenance/broken)
                    if let status = source.externalInfo?.resolvedStatus {
                        let background = status == .broken ? Color.red.opacity(0.3) : Color.yellow.opacity(0.3)
