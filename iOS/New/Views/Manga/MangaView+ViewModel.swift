...
extension MangaView {
    ...
    class ViewModel: ObservableObject {
        @Published var source: AshuraRunner.Source?

        @Published var manga: AshuraRunner.Manga
        @Published var chapters: [AshuraRunner.Chapter] = []
        @Published var otherDownloadedChapters: [AshuraRunner.Chapter] = []
        ...
        @Published var nextChapter: AshuraRunner.Chapter?
        ...
        private func setupNotifications() {
            NotificationCenter.default.publisher(for: .updateMangaDetails)
                ...
                .sink { [weak self] output in
                    Task { @MainActor in
                        guard
                            let self,
                            let manga = output.object as? AshuraRunner.Manga,
            ...
            NotificationCenter.default.publisher(for: .migratedManga)
                ...
                .sink { [weak self] output in
                    guard
                        let self,
                        let migration = output.object as? (from: AshuraRunner.Manga, to: AshuraRunner.Manga),
...
extension MangaView.ViewModel {
    ...
    func fetchDownloadedChapters() async {
        let downloadedChapters = await DownloadManager.shared.getDownloadedChapters(for: manga.identifier)
            ...
            .map { $0.toChapter() }
            .sorted { (lhs: AshuraRunner.Chapter, rhs: AshuraRunner.Chapter) in
    ...
    func refresh() async {
        ...
        do {
            ...
            // update manga in db
            if inLibrary {
                let resultManga: AshuraRunner.Manga? =
...
extension MangaView.ViewModel {
    ...
    func markRead(chapters: [AshuraRunner.Chapter]) async {
    ...
    func markUnread(chapters: [AshuraRunner.Chapter]) async {
    ...
    private func sortedChapters() -> [AshuraRunner.Chapter] {
    ...
    private func filteredChapters() -> [AshuraRunner.Chapter] {
    ...
    enum ChapterResult: Equatable {
        ...
        case allRead
        case allLocked
        case chapter(AshuraRunner.Chapter)
    }
