...
extension HistoryView {
    ...
    class ViewModel: ObservableObject {
        ...
        @Published var mangaCache: [String: AshuraRunner.Manga] = [:]
        @Published var chapterCache: [String: AshuraRunner.Chapter] = [:]
        ...
        // Ashura: manga/anime history filter
        @Published var mediaKindFilter: AppMediaKind = MediaKindPreferences.historyMediaKind {
            didSet {
                guard oldValue != mediaKindFilter else { return }
                refilterHistory()
            }
        }
...
extension HistoryView.ViewModel {
    ...
    private func loadMangaAndChapters(mangaKey: MangaKey, chapterIds: Set<String>) async {
        ...
        let tempManga = AshuraRunner.Manga(sourceKey: sourceId, key: mangaId, title: "")
...
extension HistoryView.ViewModel {
    ...
    private nonisolated func processHistoryObjects(limit: Int, offset: Int) async -> Int {
        ...
        var newMangaCacheItems: [String: AshuraRunner.Manga] = [:]
        var newChapterCacheItems: [String: AshuraRunner.Chapter] = [:]
    ...
    private func filterDay(entries: [HistoryEntry]) -> [HistoryEntry] {
        ...
        for entry in entries {
            // Ashura: heuristic media-kind filter; sources without a resolved media kind
            // are treated as manga.
            let sourceKind = SourceManager.shared.source(for: entry.sourceKey)?.mediaKind
            let resolvedKind = AppMediaKind(rawValue: sourceKind?.rawValue ?? AshuraRunner.MediaKind.manga.rawValue) ?? .manga
            guard resolvedKind == mediaKindFilter else { continue }
...
extension HistoryView.ViewModel {
    ...
    private func addMangaCacheItems(_ newItems: [String: AshuraRunner.Manga]) {
        for (key, manga) in newItems {
            mangaCache[key] = manga
        }
    }

    private func addChapterCacheItems(_ newItems: [String: AshuraRunner.Chapter]) {
