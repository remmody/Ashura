...
class TabBarController: UITabBarController {
    ...
    // Ashura: dual manga/anime library support
    private weak var libraryViewController: LibraryViewController?
    private var libraryLongPressGesture: UILongPressGestureRecognizer?
...
// MARK: - Ashura: Library Media Kind (long press to pick default library)
extension TabBarController {
    private static let libraryTabIndex = 0

    private func setUpLibraryLongPressGesture() {
        let gesture = UILongPressGestureRecognizer(target: self, action: #selector(handleTabBarLongPress(_:)))
        gesture.minimumPressDuration = 0.4
        tabBar.addGestureRecognizer(gesture)
        libraryLongPressGesture = gesture
    }
    ...
    @objc private func handleTabBarLongPress(_ gesture: UILongPressGestureRecognizer) {
        ...
        guard isLibraryTab(at: point) else { return }

        UIImpactFeedbackGenerator(style: .medium).impactOccurred()

        let alert = UIAlertController(
            title: NSLocalizedString("SELECT_DEFAULT_LIBRARY", comment: ""),
            message: nil,
            preferredStyle: .actionSheet
        )
        alert.addAction(UIAlertAction(title: NSLocalizedString("LIBRARY_MANGA", comment: ""), style: .default) { [weak self] _ in
            self?.selectDefaultLibraryMediaKind(.manga)
        })
        alert.addAction(UIAlertAction(title: NSLocalizedString("LIBRARY_ANIME", comment: ""), style: .default) { [weak self] _ in
            self?.selectDefaultLibraryMediaKind(.anime)
        })
    ...
    private func selectDefaultLibraryMediaKind(_ kind: AppMediaKind) {
        MediaKindPreferences.defaultLibraryMediaKind = kind
        MediaKindPreferences.lastLibraryMediaKind = kind
        libraryViewController?.applyMediaKindFromPreferences()
        selectedIndex = Self.libraryTabIndex
    }

    /// Rough hit-test for the library tab bar item, assuming evenly spaced tab bar buttons.
    /// Avoids relying on private tab bar button view internals.
    private func isLibraryTab(at point: CGPoint) -> Bool {
        guard let itemCount = tabBar.items?.count, itemCount > 0 else { return false }
        let itemWidth = tabBar.bounds.width / CGFloat(itemCount)
        let libraryFrame = CGRect(x: 0, y: 0, width: itemWidth, height: tabBar.bounds.height)
        return libraryFrame.contains(point)
    }
}

// MARK: - Ashura: restore last-viewed library media kind
extension TabBarController {
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        // select library content based on the last-viewed media kind on first appearance
        libraryViewController?.applyMediaKindFromPreferences()
    }
}
