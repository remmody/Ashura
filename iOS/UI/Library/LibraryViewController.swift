...
class LibraryViewController: OldMangaCollectionViewController {
    ...
    // Ashura: manga/anime library segment
    private lazy var mediaKindSegmentedControl: UISegmentedControl = {
        let control = UISegmentedControl(items: [
            NSLocalizedString("LIBRARY_MANGA", comment: ""),
...
// MARK: - Ashura: Media Kind filtering
extension LibraryViewController {
    /// Selects the segmented control based on the last-viewed library media kind and refreshes
    /// the displayed items. Safe to call before the view has loaded.
    func applyMediaKindFromPreferences() {
        guard isViewLoaded else { return }
        let kind = MediaKindPreferences.lastLibraryMediaKind
        mediaKindSegmentedControl.selectedSegmentIndex = kind == .anime ? 1 : 0
        updateDataSource()
    }

    @objc private func mediaKindSegmentChanged() {
        let kind: AppMediaKind = mediaKindSegmentedControl.selectedSegmentIndex == 1 ? .anime : .manga
        MediaKindPreferences.lastLibraryMediaKind = kind
        updateDataSource()
        updateEmptyStack()
    }

    /// Heuristic media-kind filter: keeps items whose installed source reports a matching
    /// `mediaKind`. Sources without a resolved media kind are treated as manga.
    func filteredForCurrentMediaKind(_ items: [MangaInfo]) -> [MangaInfo] {
        let selectedKind = MediaKindPreferences.lastLibraryMediaKind
        return items.filter { info in
            let sourceKind = SourceManager.shared.source(for: info.sourceId)?.mediaKind ?? .manga
            let resolvedKind = AppMediaKind(rawValue: sourceKind.rawValue) ?? .manga
            return resolvedKind == selectedKind
        }
    }
}
...
extension LibraryViewController {
    ...
    func updateDataSource() {
        ...
        if !locked {
            let pinnedManga = filteredForCurrentMediaKind(viewModel.pinnedManga)
            let regularManga = filteredForCurrentMediaKind(viewModel.manga)

            if !pinnedManga.isEmpty {
                snapshot.appendSections(Section.allCases)
                snapshot.appendItems(pinnedManga, toSection: .pinned)
            } else {
                snapshot.appendSections([.regular])
            }

            snapshot.appendItems(regularManga, toSection: .regular)
        }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if UserDefaults.standard.bool(forKey: "Library.opensReaderView") {
            Task {
                ...
                let sortedChapters: [AshuraRunner.Chapter] = {
                    switch sortOption {
                        case .sourceOrder:
