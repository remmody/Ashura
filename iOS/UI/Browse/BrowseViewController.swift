...
class BrowseViewController: BaseTableViewController {
    ...
    // Ashura: manga/anime browse segment
    private lazy var mediaKindSegmentedControl: UISegmentedControl = {
        let control = UISegmentedControl(items: [
            NSLocalizedString("MEDIA_KIND_MANGA", comment: ""),
            NSLocalizedString("MEDIA_KIND_ANIME", comment: "")
        ])
...
extension BrowseViewController {
    func uninstall(sources: [AshuraRunner.Source]) {
    ...
    // Ashura: switch between manga and anime browse catalogs
    @objc func mediaKindSegmentChanged() {
        let kind: AppMediaKind = mediaKindSegmentedControl.selectedSegmentIndex == 1 ? .anime : .manga
        MediaKindPreferences.browseMediaKind = kind
        Task {
            await viewModel.loadExternalSources()
            viewModel.loadUpdates()
            updateDataSource()
    ...
    @objc func deleteSelected() {
        confirmAction(
            ...
            let sources = self.tableView.indexPathsForSelectedRows?.compactMap { (path: IndexPath) -> AshuraRunner.Source? in
...
extension BrowseViewController {
    ...
    // Ashura: heuristic media-kind filter for installed sources; sources without a resolved
    // media kind (or that aren't installed, e.g. update entries) are treated as manga.
    func filteredForCurrentMediaKind(_ items: [SourceInfo2]) -> [SourceInfo2] {
        let selectedKind = MediaKindPreferences.browseMediaKind
        return items.filter { info in
            let sourceKind = SourceManager.shared.source(for: info.sourceId)?.mediaKind
            let resolvedKind = AppMediaKind(rawValue: sourceKind?.rawValue ?? AshuraRunner.MediaKind.manga.rawValue) ?? .manga
    ...
    func updateDataSource() {
        ...
        let updatesSources = filteredForCurrentMediaKind(viewModel.updatesSources)
        let pinnedSources = filteredForCurrentMediaKind(viewModel.pinnedSources)
        let installedSources = filteredForCurrentMediaKind(viewModel.installedSources)
