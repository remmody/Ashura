...
class BrowseViewModel {
    ...
    func loadExternalSources(reload: Bool = false) async {
        // Ashura: load the source lists matching the currently selected browse media kind
        if MediaKindPreferences.browseMediaKind == .anime {
            await SourceManager.shared.loadAnimeSourceLists(reload: reload)
        } else {
        ...
        for sourceList in SourceManager.shared.currentMediaKindSourceLists {
