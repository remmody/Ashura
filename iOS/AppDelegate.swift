...
extension AppDelegate {
                    ...
                    if !pathComponents.isEmpty { // /sourceId/mangaId
                        ...
                        guard
                            ...
                            let manga = try? await source.getMangaUpdate(
                                manga: AshuraRunner.Manga(sourceKey: source.id, key: mangaKey, title: ""),
                                needsDetails: true,
                                needsChapters: false
                            )
                        else {
    ...
    func handleDeepLink(url: URL) async -> Bool {
        ...
        var targetSource: AshuraRunner.Source?
        var finalUrl: String?
    ...
    func handleSourceMigration(source: AshuraRunner.Source) {
        presentAlert(
