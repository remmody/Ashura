...
protocol Tracker: AnyObject, Sendable {
    ...
    func search(for manga: AshuraRunner.Manga, includeNsfw: Bool) async throws -> [TrackSearchItem]
    ...
    // TODO(ashura-phase4): once anime tracking is implemented, use `supportsAnime` to filter which
    // trackers are offered/searched for anime titles (mirroring the manga tracking flow).
    /// Whether this tracker supports tracking anime titles, in addition to manga. Defaults to false;
    /// scaffolding for phase4 anime tracking support.
