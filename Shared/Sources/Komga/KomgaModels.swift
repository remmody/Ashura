...
extension KomgaBook {
    func intoManga(sourceKey: String, baseUrl: URL) -> AshuraRunner.Manga {
        .init(
            sourceKey: sourceKey,
            key: seriesId,
    ...
    func intoChapter(baseUrl: URL, useChapters: Bool) -> AshuraRunner.Chapter {
        .init(
            key: id,
            title: metadata.title,
...
extension KomgaSeries {
    func intoManga(sourceKey: String, baseUrl: URL) -> AshuraRunner.Manga {
        let status: AshuraRunner.PublishingStatus = switch metadata.status {
            case .ended: .completed
            case .ongoing: .ongoing
            case .abandoned: .cancelled
            case .hiatus: .hiatus
        }
        let contentRating: AshuraRunner.ContentRating = metadata.ageRating.flatMap {
            if $0 >= 18 {
                .nsfw
            } else if $0 >= 16 {
                .suggestive
        ...
        let viewer: AshuraRunner.Viewer = switch metadata.readingDirection {
