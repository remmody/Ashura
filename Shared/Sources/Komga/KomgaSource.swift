...
actor KomgaSourceRunner: Runner {
    ...
    func getSearchFilters() async throws -> [AshuraRunner.Filter] {
        ...
        var filters: [AshuraRunner.Filter] = []
    ...
    func getListings() async throws -> [AshuraRunner.Listing] {
        var lastWorkingMirrorCopy = lastWorkingMirror
        defer {
            lastWorkingMirror = lastWorkingMirrorCopy
        }
...
extension KomgaSourceRunner {
    ...
    func getHome() async throws -> Home {
        ...
        try await withThrowingTaskGroup(of: (Int, AshuraRunner.Listing, [HomeComponent.Value.Link]).self) { [helper, sourceKey] taskGroup in
            for (index, listingType) in listingTypes.enumerated() {
                taskGroup.addTask { [self, lastWorkingMirror] in
                    let listing = AshuraRunner.Listing(id: listingType.id, name: listingType.name, kind: .default)
    ...
    func getListingHome(listing: AshuraRunner.Listing) async throws -> Home? {
        let baseUrl = try helper.getConfiguredServer()
        ...
        try await withThrowingTaskGroup(of: (Int, AshuraRunner.Listing, [HomeComponent.Value.Link]).self) { [helper, sourceKey] taskGroup in
            for (index, listingType) in listingTypes.enumerated() {
                taskGroup.addTask { [self, lastWorkingMirror] in
                    ...
                    let componentListing = AshuraRunner.Listing(
