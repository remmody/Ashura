...
extension AshuraRunner.Source {
    ...
    static func legacy(source: Source) -> AshuraRunner.Source {
        AshuraRunner.Source(
            url: source.url,
            key: source.id,
            ...
            contentRating: AshuraRunner.SourceContentRating(rawValue: source.manifest.info.nsfw ?? 0) ?? .safe,
            ...
            staticListings: source.listings.map { AshuraRunner.Listing(id: $0.name, name: $0.name) },
            staticFilters: [],
...
final class LegacySourceRunner: AshuraRunner.Runner {
    let source: Source
    ...
    func getHome() async throws -> AshuraRunner.Home {
        throw AshuraRunner.SourceError.unimplemented
    }
    ...
    func getImageRequest(url: String, context _: AshuraRunner.PageContext?) async throws -> URLRequest {
