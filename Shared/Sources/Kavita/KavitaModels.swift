...
extension KavitaSeries {
    func intoManga(
        ...
        let status: AshuraRunner.PublishingStatus = switch metadata?.publicationStatus {
        ...
        let contentRating: AshuraRunner.ContentRating = if let ageRating = metadata?.ageRating {
            if ageRating >= 10 {
                // mature 17+ or r18
                .nsfw
            } else if ageRating >= 8 {
...
extension KavitaVolume {
    func intoChapters(baseUrl: URL, apiKey: String) -> [AshuraRunner.Chapter] {
        chapters.compactMap { chapter -> AshuraRunner.Chapter? in
