...
extension LocalFileDataManager {
    ...
    func fetchLocalSeries(query: String? = nil) -> [AshuraRunner.Manga] {
        let request = MangaObject.fetchRequest()
        request.predicate = if let query, !query.isEmpty {
            NSPredicate(format: "sourceId == %@ AND title CONTAINS[cd] %@", LocalSourceRunner.sourceKey, query)
        } else {
    ...
    func fetchLocalSeries(id: String) -> AshuraRunner.Manga? {
        let request = MangaObject.fetchRequest()
        request.predicate = NSPredicate(format: "id == %@ AND sourceId == %@", id, LocalSourceRunner.sourceKey)
        request.fetchLimit = 1
        do {
    ...
    func fetchChapters(mangaId: String) -> [AshuraRunner.Chapter] {
        let request = ChapterObject.fetchRequest()
...
extension LocalFileDataManager {
    func createManga(
        ...
        viewer: AshuraRunner.Viewer = .unknown,
        ...
        if let comicInfo {
            let basicManga = AshuraRunner.Manga(
    ...
    func createChapter(
        ...
        if let comicInfo {
            let basicChapter = AshuraRunner.Chapter(
                key: id,
                title: title,
