...
extension LocalFileManager {
    ...
    func fetchPages(mangaId: String, chapterId: String) async -> [AshuraRunner.Page] {
    ...
    nonisolated func readEpubPages(from archiveURL: URL, chapterId: String) -> [AshuraRunner.Page] {
        ...
        if hasText {
            ...
            return [AshuraRunner.Page(content: .text(parts.joined(separator: "\n\n")))]
        }

        // image-only chapter (cover, illustration pages)
        return segments.compactMap { segment in
            guard case let .image(path) = segment else { return nil }
            return AshuraRunner.Page(content: .zipFile(url: archiveURL, filePath: path))
        }
    }
    ...
    nonisolated func readPages(from archiveURL: URL) -> [AshuraRunner.Page] {
        let archive: Archive
        do {
        ...
        var pages = archive
            ...
            .map { entry in
                AshuraRunner.Page(content: .zipFile(url: archiveURL, filePath: entry.path))
            }
