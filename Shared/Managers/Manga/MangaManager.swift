...
actor MangaManager {
    ...
    nonisolated func getNextChapter(
        ...
        chapters: [AshuraRunner.Chapter],
        readingHistory: [String: (page: Int, date: Int)],
        sortAscending: Bool
    ) -> AshuraRunner.Chapter? {
        ...
        var selectedChapter: AshuraRunner.Chapter?
...
extension MangaManager {
    ...
    private func doLibraryRefresh(
        ...
        let newDetails = await {
            var results: [Int: AshuraRunner.Manga] = [:]
...
extension MangaManager {
    ...
    func setCover(manga: AshuraRunner.Manga, cover: PlatformImage) async -> String? {
        if manga.isLocal() {
    ...
    func resetCover(manga: AshuraRunner.Manga) async -> String? {
...
extension MangaManager {
    func migrate(
        copy: Bool,
        fromSeries: [AshuraRunner.Manga],
        toSeries: [MangaIdentifier: AshuraRunner.Manga?],
        withChapters: [MangaIdentifier: [AshuraRunner.Chapter]] = [:],
        ...
        await withTaskGroup(of: (from: AshuraRunner.Manga, to: AshuraRunner.Manga)?.self) { group in
            let batchSize = 10
    ...
    private static func migrate(
        copy: Bool,
        from oldManga: AshuraRunner.Manga,
        to newManga: AshuraRunner.Manga,
        withChapters newChapters: [AshuraRunner.Chapter],
    ) async -> (AshuraRunner.Manga, AshuraRunner.Manga)? {
        // migrate settings
    ...
    ) async -> [String: (AshuraRunner.Manga, [AshuraRunner.Chapter])] {
        await withTaskGroup(
            of: (String, AshuraRunner.Manga, [AshuraRunner.Chapter])?.self,
            returning: [String: (AshuraRunner.Manga, [AshuraRunner.Chapter])].self
        ) { group in
            let batchSize = 10
            var ret: [String: (AshuraRunner.Manga, [AshuraRunner.Chapter])] = [:]
            var counter = 0
