        ...
        .init(
            ...
            requestHandler: { originalRequest in
                let request = if let url = originalRequest.url {
                    await AshuraRunner.Source.modify(url: url, request: originalRequest)
                } else {
                    originalRequest
                }
...
extension AshuraRunner.PublishingStatus {
    var title: String {
        switch self {
...
extension AshuraRunner.ContentRating {
    var title: String {
        switch self {
            case .unknown: NSLocalizedString("UNKNOWN")
...
extension AshuraRunner.SourceContentRating {
    var title: String {
        switch self {
            case .safe: NSLocalizedString("SAFE")
...
extension AshuraRunner.SelectFilter {
