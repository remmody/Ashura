...
public class MangaObject: NSManagedObject {
    ...
    func load(from manga: AshuraRunner.Manga, sourceId: String? = nil, override: Bool = false) {
        let editedKeys = EditedKeys(rawValue: editedKeys)
        id = manga.key
    ...
    func toNewManga() -> AshuraRunner.Manga {
        ...
        return AshuraRunner.Manga(
            ...
            tags: tags,
            status: AshuraRunner.PublishingStatus(rawValue: UInt8(status)) ?? .unknown,
            contentRating: AshuraRunner.ContentRating(rawValue: UInt8(nsfw + 1)) ?? .unknown,
            viewer: viewer,
            updateStrategy: neverUpdate ? .never : .always,
