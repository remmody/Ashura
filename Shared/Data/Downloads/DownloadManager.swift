...
actor DownloadManager {
    ...
    func getDownloadedPages(for chapter: ChapterIdentifier) async -> [AshuraRunner.Page] {
        ...
        if archiveURL.exists {
            ...
            var pages = directory.contents
                ...
                .compactMap { url -> AshuraRunner.Page? in
                    ...
                    if LocalFileManager.allowedTextExtensions.contains(url.pathExtension) {
                        ...
                        return AshuraRunner.Page(content: .text(text))
                    } else if LocalFileManager.allowedImageExtensions.contains(url.pathExtension) {
                        // load file as image
                        return AshuraRunner.Page(content: .url(url: url, context: nil))
                    } else {
                        return nil
                    }
                }
...
extension DownloadManager {
    ...
    func downloadUnread(manga: AshuraRunner.Manga) async {
    ...
    func download(manga: AshuraRunner.Manga, chapters: [AshuraRunner.Chapter]) async {
...
extension DownloadManager {
    ...
    func saveChapterMetadata(manga: AshuraRunner.Manga, chapter: AshuraRunner.Chapter, to directory: URL) {
