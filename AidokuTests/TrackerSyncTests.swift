...
actor TestableTracker: Tracker {
    ...
    func search(for manga: AshuraRunner.Manga, includeNsfw: Bool) async throws -> [Aidoku.TrackSearchItem] {
        []
    }
...
@Suite struct TrackerSyncTests {
    ...
    static let testManga: AshuraRunner.Manga = .init(sourceKey: "test", key: "test", title: "Test")
    ...
    @Test func testVolumeNumbers() async {
        ...
        let chapters: [AshuraRunner.Chapter] = (1...10).map {
            .init(
                key: "\($0)",
                title: "\($0)",
                chapterNumber: Float($0),
